...
wget https://raw.githubusercontent.com/robomeister/crds/master/deploy-ace-fwk.json
wget https://raw.githubusercontent.com/robomeister/crds/master/deploy-ace-fwk-prod.json

case $NAMESPACE in
  *"pt"*|*"prod"*|*"dr"*)
    echo "Using PRODUCTION deployment json"
    cp deploy-ace-fwk-prod.json deploy.json
    ;;
  *)
    echo "Using non-prod deployment json"
    cp deploy-ace-fwk.json deploy.json
    ;;    
esac

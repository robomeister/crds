...
wget https://raw.githubusercontent.com/robomeister/crds/master/deploy-ace-wms.json
wget https://raw.githubusercontent.com/robomeister/crds/master/deploy-ace-wms-prod.json

case $NAMESPACE in
  *"pt"*|*"prod"*|*"dr"*)
    echo "Using PRODUCTION deployment json"
    cp deploy-ace-wms-prod.json deploy.json
    ;;
  *)
    echo "Using non-prod deployment json"
    cp deploy-ace-wms.json deploy.json
    ;;    
esac

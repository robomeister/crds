...
wget https://raw.githubusercontent.com/robomeister/crds/master/deploy-ace-pco.json
wget https://raw.githubusercontent.com/robomeister/crds/master/deploy-ace-pco-prod.json

case $NAMESPACE in
  *"pt"*|*"prod"*|*"dr"*)
    echo "Using PRODUCTION deployment json"
    cp deploy-ace-pco-prod.json deploy.json
    ;;
  *)
    echo "Using non-prod deployment json"
    cp deploy-ace-pco.json deploy.json
    ;;    
esac

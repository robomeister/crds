...
wget https://raw.githubusercontent.com/robomeister/crds/master/deploy-ace-tms.json
wget https://raw.githubusercontent.com/robomeister/crds/master/deploy-ace-tms-prod.json

case $NAMESPACE in
  *"pt"*|*"prod"*|*"dr"*)
    echo "Using PRODUCTION deployment json"
    cp deploy-ace-tms-prod.json deploy.json
    ;;
  *)
    echo "Using non-prod deployment json"
    cp deploy-ace-tms.json deploy.json
    ;;    
esac
...
set -x
if oc rollout status deploy/${DEPLOYMENT_NAME} --watch=true --request-timeout="1150s" --namespace ${NAMESPACE}; then
  STATUS="pass"
else
  STATUS="fail"
fi
